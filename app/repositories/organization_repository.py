...
class OrganizationRepository(BaseRepo):
    ...
    async def checking_existence_organization(self, name_org: str):
    ...
    async def get_list_obj_for_user(self, user_id: int):
        self.log.info("get_list_obj")
        stmt_exercise = (
            select(self.main_model)
            .options(selectinload(self.main_model.members))
            .where(self.main_model.members.any(user_id=user_id))
        )
